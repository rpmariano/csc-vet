...
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    -- Janela em que o jogador deve pagar quota; sem data definida, infere-se do
    -- estado do perfil (ver src/lib/finance.ts).
...
-- 7.1 Categorias de despesa (Bolas, Coletes, Material Médico, ...) — definida
-- antes de transactions, que a referencia.
...
CREATE TABLE IF NOT EXISTS public.transactions (
    ...
    created_by UUID REFERENCES public.profiles(id) ON DELETE SET NULL,
    -- Categoria de despesa, documento comprovativo, e ligação opcional a uma
    -- tranche de inscrição de torneio (uma despesa por cada tranche paga).
...
-- 9.1 Definições financeiras do clube (linha única, tal como club_settings).
...
-- 9.2 Pagamentos de seguro desportivo — registo em vez de um único booleano,
...
-- 8.1 Políticas para o Módulo Financeiro (definições, seguro, categorias de despesa)
...
ON public.financial_settings FOR ALL TO authenticated USING (public.get_user_role() = 'admin');
...
ON public.expense_categories FOR ALL TO authenticated USING (public.get_user_role() = 'admin');

-- Bucket privado para documentos comprovativos de despesa
