...
- **Vaga 3 — feita.** "Eliminar" em botões, títulos e confirmações ("Eliminar encargo" / "Sim, eliminar encargo"), "Tirar" para pertenças (convocatória, grupo); `toast.comAnular()` ao tirar um atleta e ao desmarcar uma quota; confirmação ao eliminar uma categoria; `mensagemDeErro()` em ~60 toasts (sem o `error.message` do Supabase em inglês); vibração dupla fora; "A guardar…" único e "Guardar e sair / Sair sem guardar / Continuar a editar"; estado de gravação nos Dados do clube, Campos e registo de pagamento de encargo (este gravava duas vezes com dois toques); "tu" em vez de "você" nas mensagens.
  Ficou de fora, por decisão: a validação dupla (`required` + toast) — o toast é código morto mas não se vê; e o `ConfirmModal` a mostrar progresso enquanto elimina, que obrigava a mexer em 15 sítios que fecham o diálogo antes de esperar.
- **Vaga 4 — feita.** Primitivos novos em `components/ui`: `BotaoIcone`, `Interruptor`, `ACarregar`, `EstadoVazio`, `formulario.ts` (`CLASSE_CAMPO`, `CLASSE_ETIQUETA_CAMPO`) e a aparência `vermelho` do `Botao`. Aplicados: 13 cópias do campo e 10 do rótulo substituídas, mais os 14 campos de ~32px do formulário de torneio; 4 interruptores; 16 botões de ícone (Gestões, Liga, Eventos, Classificação, Financeiro, Comunicados); botões de editar/eliminar das fichas; 12 rodopios (incluindo o azul-marinho invisível do `ProtectedRoute`) e 9 vazios (o das Fichas passa a saber que há filtros); 5 diálogos de formulário passaram ao `<Modal>` (criar campo, criar adversário, campo e adversário rápidos, editar comunicado); as três confirmações partilhadas usam `<Botao>`; `EtiquetaSeccao` alinhada com o `CLAUDE.md` (0.16em, /70) e os rótulos das persianas de filtros com o dos campos.
  Fica para a vaga 5: os ~96 cartões e ~200 botões à mão restantes, as cores fora dos tokens, os `sm:` mortos e os filtros de estado à vista — é migração ecrã a ecrã.
- **Vaga 5 — feita, com três exceções deliberadas.** 66 prefixos `sm:`/`md:` mortos fora; 115 cores da paleta do Tailwind passadas aos tokens (toasts reescritos, `CORES_TIPO` em `lib/eventos` e usado no dossier dos Eventos); `ProcuraEFiltros` novo, adotado nos Comunicados (estado para o funil), nos Torneios (idem), nos Adversários, nos Campos e nos Movimentos (período para o funil, procura nova, grupos em caixa com banda); `fmtData` em `lib/datas` para as datas sem hora (10 sítios) e `fmtEuro` nos 3 sítios que o reimplementavam. `procura-e-filtros.spec.ts` cobre o padrão novo.
  **Exceções:** o mosaico de estados do Plantel continua a ser filtro à vista (é o ecrã 3a do handoff, e a decisão está documentada no código); as paletas dos gráficos continuam categóricas; e a conversão dos ~96 cartões e ~200 botões restantes aos primitivos não se fez em bloco — é sobretudo código, e o visual já coincide. Faz-se quando cada ecrã for tocado, como manda o `CLAUDE.md`.
